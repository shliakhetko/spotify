  ...
  return (
    <div className='w-1/3 min-w-[20rem] w-full flex justify-center items-center flex-col'>
      <div className='w-fit flex justify-between items-center space-x-2 '>
        <button className={classNames(state.remix ? 'text-green-500' : 'text-neutral-300','p-2')} onClick={() => {
          dispatch({ 'type': PlayerActionTypes.REMIX });
        }}><FaRandom /></button>
        <button className={classNames('text-neutral-300','p-2')} onClick={() => {
          dispatch({ 'type': PlayerActionTypes.PLAY_PREVIOUS });
        }}><FaBackwardStep /></button>
        <button className={classNames('rounded-full text-black bg-white','p-3')} onClick={() => {
          setPlaying(!playing);
        }}>{playing ? <FaPause /> : <FaPlay />}</button>
        <button className={classNames('text-neutral-300','p-2')} onClick={() => {
          dispatch({ 'type': PlayerActionTypes.PLAY_PREVIOUS });
        }}><FaForwardStep /></button>
        <button className={classNames(state.loop != 0 ? 'text-green-500' : 'text-neutral-300', "relative",'p-2')} onClick={() => {
          dispatch({ 'type': PlayerActionTypes.LOOP });
        }}><SlLoop />{state.loop == 2 && <span className='absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-2/5 text-[7px]'>1</span>}</button>
      </div>
      <div className='w-2/3 flex justify-between items-center space-x-3'>
        <span className='text-neutral-200 text-sm'><Duration seconds={duration * played} /></span>
        <input
          id="slider"
          className="w-full"
          type="range"
        ...
        />
        <span className='text-neutral-200 text-sm'><Duration seconds={duration} /></span>
      </div>
      {<ReactPlayer
        height={0}
        width={0}
        ref={playerRef}
        url={state.playing || undefined}
        volume={state.volume}
        playing={playing}
        onPlay={handlePlay}
        onPause={handlePause}
